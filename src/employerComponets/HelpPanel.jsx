...
import { submitHelpMessage, listenToMyHelpMessages } from "@/lib/Helpqueries";
    ...
    <div className="fixed inset-0 z-10001 flex items-center justify-center px-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[85vh] overflow-y-auto">
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          <div>
            <p className="text-[10px] font-bold text-[#004AAC] uppercase tracking-wide mb-1.5">
              {item.repliedBy || "Support Team"}
            </p>
            <div className="bg-[#004AAC]/5 border border-[#004AAC]/20 rounded-xl px-4 py-3">
              <p className="text-sm text-slate-800 whitespace-pre-line">
                {item.reply}
              </p>
              <p className="text-[10px] text-slate-400 mt-2">
                {timeAgo(item.repliedAt)}
              </p>
            </div>
          </div>
        </div>
...
export default function HelpPanel({
  ...
  const [submittedType, setSubmittedType] = useState("query");
  ...
  const [viewReplyFor, setViewReplyFor] = useState(null);
  ...
  return createPortal(
    <>
          ...
          {!authed ? (
            <div className="flex flex-col items-center text-center py-10 px-4">
              ...
              <p className="text-sm text-slate-500 mb-5 max-w-xs">
                Sign in or create an account so we know where to send our reply.
              </p>
              <button
            ...
            <>
              <div className="grid grid-cols-2 gap-2 mb-5">
                <button
                  ...
                  className={cls(
                    "py-2.5 rounded-xl text-sm font-bold transition-all border disabled:opacity-60",
                    mode === "query"
                    ...
                    {mine.slice(0, 6).map((m) => (
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between gap-2 mt-1.5">
                          ...
                          {m.reply && (
                            <button
                              onClick={() => setViewReplyFor(m)}
                              className="flex items-center gap-1 text-[11px] font-bold text-[#004AAC] hover:underline shrink-0"
                            >
                              <MessageCircle size={11} /> Read Reply
                            </button>
                          )}
                        </div>
                      </div>
      ...
      <ReplyModal item={viewReplyFor} onClose={() => setViewReplyFor(null)} />
    </>,
    document.body,
  );
}
